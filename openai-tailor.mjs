...
╚══════════════════════════════════════════════════════════════════╝
  ...
  USAGE
    ...
    node openai-tailor.mjs --provider grok --jd <path> --report <path>
...
let jdPath     = '';
let reportPath = '';
let modelName  = providerConfig.id === 'openai-compatible' && !process.env.OPENAI_MODEL
  ? 'gpt-4o'
...
❌  No API key for ${endpointHost}.
   Set one and re-run: ${providerConfig.apiKeyEnv}=your_key node openai-tailor.mjs${providerConfig.id === 'grok' ? ' --provider grok' : ''} ...
`);
    process.exit(1);
  }
}
